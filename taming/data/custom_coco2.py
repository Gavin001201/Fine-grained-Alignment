...
class AnnotatedObjectsDataset(Dataset):
    def __init__(self, data_path: Union[str, Path], split: SplitType, keys: List[str], target_image_size: int,
                 crop_method: CropMethodType, random_flip: bool, use_group_parameter: bool,
                 encode_crop: bool, category_allow_list_target: str = "", category_mapping_target: str = "",
                 no_object_classes: Optional[int] = None, context_length: int = 77):
        ...
        self.image_ids = None
        self.transform_functions: List[Callable] = self.setup_transform(target_image_size, crop_method, random_flip)    #图像的数据增强
        self.paths = self.build_paths(self.data_path)   #self.data_path， 'data/coco_annotations_100'，得到数据路径
    ...
    def build_paths(self, top_level: Union[str, Path]) -> Dict[str, Path]:
        top_level = Path(top_level)     #WindowsPath('data/coco_annotations_100')， 配置文件来的
    ...
    def __getitem__(self, n: int) -> Dict[str, Any]:
        # image_id = self.get_image_id(n)     #拿到标注信息的id
        image_id = list(self.annotations[n].keys())[0]
        sample = self.get_image_description(image_id)       #n拿到对应的图片信息
        # sample['annotations'] = self.get_annotation(image_id)
        sample['caption'] = self.annotations[n][image_id].caption
        sample['nl_text'] = sample['caption']
        sample['caption'] = tokenize(sample['caption'], context_length=self.context_length)

        if 'image' in self.keys:
            sample['image_path'] = str(self.get_image_path(image_id))   #拿到图片
            sample['image'] = self.load_image_from_disk(sample['image_path'])   #转成RGB
            sample['image'] = convert_pil_to_tensor(sample['image'])            #转成tensor
            _, _, sample['image'] = self.image_transform(sample['image'])
            _, w, h = sample['image'].shape
            coord = np.arange(h*w).reshape(h,w,1)/(h*w)
            sample['image'] = sample['image'].permute(1, 2, 0)
            sample['file_name'] = self.image_descriptions[str(image_id)].file_name
            sample['coord'] = coord
