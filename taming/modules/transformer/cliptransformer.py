...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(
        ...
        v_x = v_x if v_x is not None else q_x       #k_x == q_x == v_x
    ...
    def forward(
        ...
        k_x = self.ln_1_kv(k_x) if hasattr(self, "ln_1_kv") and k_x is not None else None   #None
        v_x = self.ln_1_kv(v_x) if hasattr(self, "ln_1_kv") and v_x is not None else None   #None

        x = q_x + self.ls_1(self.attention(q_x=self.ln_1(q_x), k_x=k_x, v_x=v_x, attn_mask=attn_mask))      #q_x == x
...
class Transformer(nn.Module):
    def __init__(
            ...
            layers: int,
            heads: int,
            mlp_ratio: float = 4.0,                 #全连接相对输入维度的倍数
            ls_init_value: float = None,
            act_layer: Callable = nn.GELU,
            norm_layer: Callable = LayerNorm,
    ):
...
class TextTransformer(nn.Module):
    ...
    def forward(self, text, valid_lens):
        ...
        attn_mask = self.attn_mask                     # 简单的三角矩阵

        # 改写，对padding部分不进行关注
        pad_mask = (text != self.pad_id).unsqueeze(1).to(attn_mask.device)
        ...
        x = x.permute(1, 0, 2)  # LND -> NLD                # [8, 256, 256]
        x = self.ln_final(x)                            # [8, 256, 256]
        return x, mask          #[bs, l , d]
