...
class Downsample(nn.Module):                #下采样两倍
    def __init__(self, in_channels, with_conv):
        super().__init__()
        self.with_conv = with_conv
        if self.with_conv:
...
class ResnetBlock(nn.Module):
    def __init__(self, *, in_channels, out_channels=None, conv_shortcut=False,
        ...
        self.norm1 = Normalize(in_channels)             #torch.nn.GroupNorm
        ...
        if temb_channels > 0:
            self.temb_proj = torch.nn.Linear(temb_channels,
                                             out_channels)
        self.norm2 = Normalize(out_channels)            #torch.nn.GroupNorm
...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        # downsampling
        self.conv_in = torch.nn.Conv2d(in_channels,         #in_channels:3,  ch:128, 卷积前后大小不变
                                       self.ch,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
        ...
        for i_level in range(self.num_resolutions):     #5
            ...
            block_out = ch*ch_mult[i_level]     #128*[1, 1, 2, 2, 4]        out_channels = in_channels if out_channels is None else out_channels
            for i_block in range(self.num_res_blocks):  #2
                block.append(ResnetBlock(in_channels=block_in,              #只改变通道数
                                         out_channels=block_out,
                                         temb_channels=self.temb_ch,
                                         dropout=dropout))
                block_in = block_out
            ...
            down.attn = attn
            if i_level != self.num_resolutions-1:
                down.downsample = Downsample(block_in, resamp_with_conv)    #kernelsize:3, stride: 2    下采样两倍
                curr_res = curr_res // 2
            self.down.append(down)
    ...
    def forward(self, x):       #torch.Size([1, 3, 683, 1024])
        ...
        # downsampling
        hs = [self.conv_in(x)]  #torch.Size([1, 128, 683, 1024])
        for i_level in range(self.num_resolutions):     #2
            for i_block in range(self.num_res_blocks):  #2
                h = self.down[i_level].block[i_block](hs[-1], temb)
                if len(self.down[i_level].attn) > 0:
                    h = self.down[i_level].attn[i_block](h)
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        # upsampling
        self.up = nn.ModuleList()
        for i_level in reversed(range(self.num_resolutions)):       #反向迭代[4, 3, 2, 1, 0]
            block = nn.ModuleList()
            attn = nn.ModuleList()
            block_out = ch*ch_mult[i_level]                         # 128*[4, 3, 2, 1, 0]
            for i_block in range(self.num_res_blocks+1):
                block.append(ResnetBlock(in_channels=block_in,
                                         out_channels=block_out,
                                         temb_channels=self.temb_ch,
    ...
    def forward(self, z, key=None):
        ...
        self.last_z_shape = z.shape     # [8, 256, 16, 16]

        # timestep embedding
        temb = None

        # z to block_in
        h = self.conv_in(z)             # [8, 512, 16, 16]

        # middle
        h = self.mid.block_1(h, temb)   # [8, 512, 16, 16]
        h = self.mid.attn_1(h)          # [8, 512, 16, 16]
        h = self.mid.block_2(h, temb)   # [8, 512, 16, 16]
...
class Text_Decoder(nn.Module):
    ...
    def forward(self, hidden):
        hidden = self.text_conv(hidden)                       # [8, 256, 16, 16]
        b, d, h, w = hidden.size()
        hidden = hidden.reshape(b, d, -1)          # [8, 256, 256]
        hidden = hidden.permute(2, 0, 1)                      # [256, 8, 256]
